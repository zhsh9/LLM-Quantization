...
class W8A8Linear(nn.Module):
    def __init__(
        ...
        if act_quant == "per_token":
            ...
            self.act_quant = partial(quantize_activation_per_token_absmax, n_bits=activate_bit)
    ...
    def from_float(
        ...
        if weight_quant == "per_channel":
            new_module.weight = quantize_weight_per_channel_absmax(
                module.weight, n_bits=weight_bit
            )
    ...
    def __repr__(self):
        return f"W8A8Linear({self.in_features}, {self.out_features}, bias={self.bias is not None}, weight_quant={self.weight_quant_name}, act_quant={self.act_quant_name}, output_quant={self.output_quant_name}, weight_bit={weight_bit}, activate_bit={activate_bit})"
...
def quantize_Qwen3_like(
    ...
    for name, m in model.model.named_modules():
        if isinstance(m, (Qwen3MLP, MistralMLP)):
        ...
        elif isinstance(m, (Qwen3Attention, MistralAttention)):
            # Her we simulate quantizing BMM inputs by quantizing the output of q_proj, k_proj, v_proj
            m.q_proj = W8A8Linear.from_float(
                m.q_proj,
                weight_quant=weight_quant,
...
def quantize_model(
    ...
    elif isinstance(model, (Qwen3PreTrainedModel, MistralPreTrainedModel)):
        return quantize_Qwen3_like(
            model,
            weight_quant=weight_quant,
            act_quant=act_quant,
            quantize_bmm_input=quantize_bmm_input,
